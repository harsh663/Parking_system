...
class Entry{
    static c=0;
    constructor(capacity,owner,car,licensePlate,entryDate,exitDate){
        this.capacity=capacity-localStorage.length;
        this.owner = owner;
        this.car = car;
...
class UI{
    ...
    static validateInputs(){
        ...
        {
            UI.showAlert('capacity must be filled with intiger value','danger');
            return false;
        }
        ...
        if(!licensePlateRegex.test(licensePlate)){
            UI.showAlert('License Plate must be like AA-11-AA-1111','danger');
            return false;
        }
        return true;
    }
...
//Event Add
    ...
    document.querySelector('#entryForm').addEventListener('submit',(e)=>{
        ...
        //Instatiate Entry
        //const Capacity=new Capacity(capacity); 
        const entry = new Entry(capacity,owner, car, licensePlate, entryDate, exitDate);
        //Add the entry do de UI table
          if(Entry.c!=capacity&&Entry.c>=0)
        // if(localStorage[entry].length<=Entry.capacity)
        {
        ...
        Entry.c++;
            ...
            //   UI.showAlert('Parking is full..!!','danger');
